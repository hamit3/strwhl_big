...
#define DEVICE_NAME "STRWHL_BIG" //{"Group":"General","Name":"DEVICE_NAME","Title":"Device name,\r\n make sure to use a unique name when using multiple arduinos","DefaultValue":"SimHub Dash","Type":"string","Template":"#define DEVICE_NAME \"{0}\""}
#define DEVICE_UNIQUE_ID "07e2ee43-3a21-48d4-a193-f8b13e54ec2d" //{"UniqueId":"07e2ee43-3a21-48d4-a193-f8b13e54ec2d","Name":"DEVICE_UNIQUE_ID","Type":"uniqueid"}
...
// 0 disabled, > 0 enabled
#define WS2812B_RGBLEDCOUNT 15       //{"Group":"WS2812B RGB Leds","Name":"WS2812B_RGBLEDCOUNT","Title":"WS2812B RGB leds count","DefaultValue":"0","Type":"int","Max":150}
...
volatile boolean f500ms = LOW;

volatile boolean fAdjustModeOn = false;
volatile boolean fFlagUpdate = LOW;
volatile boolean fcheckHoldStatus = LOW;
volatile boolean fStoreCluchAdj = LOW;
...
volatile int holdcnt100ms = 0;
volatile int counterHold = 0;
volatile int lvdcnt = 0;
volatile int lvdcnttimer = 0;
...
byte rowPins[ROWS] = {8, 9, 5, 6}; //connect to the row pinouts of the keypad
byte colPins[COLS] = {13, 7, 10, 4}; //connect to the column pinouts of the keypad
...
{
  f_sw_side_push = true;
  gp.setButtonState(21, true);
}
void sw_a_rlsd(void)
{
  f_sw_side_push = false;
  gp.setButtonState(21, false);
}
...
void read_encoder_right(void) {
  ...
  {  
    if (digitalRead(ENC_RIGHT_B) == HIGH) {
        gp.setButtonState(23, true);
        delay(50);
        gp.setButtonState(23, false);
    } 
    else 
    {
        gp.setButtonState(22, true);
        delay(50);
        gp.setButtonState(22, false);
    }
  }
...
void read_encoder_left(void) {
  ...
  {  
    if (digitalRead(ENC_LEFT_B) == HIGH) {
        gp.setButtonState(25, true);
        delay(50);
        gp.setButtonState(25, false);
    } 
    else 
    {
        gp.setButtonState(24, true);
        delay(50);
        gp.setButtonState(24, false);
    }
  }
...
void sw_read_encoder(void) {
  ...
  {  
    if (digitalRead(SW_ENC_B) == HIGH) {
        gp.setButtonState(27, true);
        delay(50);
        gp.setButtonState(27, false);
    } 
    else 
    {
        gp.setButtonState(26, true);
        delay(50);
        gp.setButtonState(26, false);
    }
  }
...
void read_button_matrix(void)
{
  char keych;
    ...
    if (keypad.getKeys())
    {         
        for (int j=0; j<16; j++)   // Scan the whole key list.
        {
            if ( keypad.key[j].stateChanged )   // Only find keys that have changed state.
            {
                switch (keypad.key[j].kstate) {  // Report active key state : IDLE, PRESSED, HOLD, or RELEASED
                    case PRESSED:
                    msg = " PRESSED.";
                    lvdcnttimer = 0;
                    keych = keypad.key[j].kchar;
                    for(int i = 1; i < 10; i ++)
                    {
                      if(keych == (int)'0' + i)
                      {
                ...
                //Serial.print("Key ");
                //Serial.print(keypad.key[j].kchar);
                //Serial.println(msg);
            }
        }
    }
...
{
   int8_t lx, ly, rx, ry;
   Val1=analogRead(A0);            //We read both values and display them raw on the serial monitor
   //gp.setLeftClutch(Val1);
   Val2 = Val1-680;
   if(Val1 < 0 )
   {
    Val1 = 0;
...
void adjust_loop(void)
{
  for(int i = 0; i < 15; i ++)
  {
    if((i < 3) || (i > 11))
    {
      shRGBLedsWS2812B.setPixelColor(i, 255, 255, 0);
    }
    else
    {
      if( i < Val1/10)
      {
        shRGBLedsWS2812B.setPixelColor(i, 255, 0, 0);      
      }
      else
      {
        shRGBLedsWS2812B.setPixelColor(i, 0, 0, 0);
      }
    }
    if(fFlagUpdate)
    {
      shRGBLedsWS2812B.setPixelColor(2, 255, 255, 255);
      shRGBLedsWS2812B.setPixelColor(12, 255, 255, 255);
    }
    else
    {
      shRGBLedsWS2812B.setPixelColor(2, 0, 0, 0);
      shRGBLedsWS2812B.setPixelColor(12, 0, 0, 0);      
    }
  }
  FastLED.show();
}
...
void storeCluch(void)
{
  runSingleColor(CRGB::Blue);
  delay(500);
  runSingleColor(CRGB::Green);
  delay(500);
  runSingleColor(CRGB::White); 
  delay(500);
  runSingleColor(CRGB::Black);   
}
